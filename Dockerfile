...
RUN echo '*/5 * * * * cd ${APP_WORKDIR} && ${PYTHON_VER_PATH} ${APP_WORKDIR}/${APP_NAME} --config ${APP_WORKDIR}/${APP_CONFIG} --ioc-manager-db-path /db/ioc.db > /proc/1/fd/1 2>/proc/1/fd/2' >> crontab
RUN crontab crontab
